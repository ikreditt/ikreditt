...
class _DraggableHomeState extends State<DraggableHome> {
  ...
  listenOnScroll() {
    widget.scrollController?.addListener(() {
      if (widget.scrollController != null && !widget.leadingPressed) {
        snapPart();
      }
    });
  }
  ...
  void snapPart() {
    if (widget.scrollController!.offset.clamp(0, getAppBarHeight()) ==
        widget.scrollController!.offset) {
      var pos = widget.scrollController!.offset;
      var half = getAppBarHeight() - getAppBarHeight() / 2;
      if (pos <= half) {
        widget.scrollController!.animateTo(0,
            duration: const Duration(milliseconds: 50), curve: Curves.linear);
      } else if (widget.scrollController!.offset >= getAppBarHeight() / 2) {
